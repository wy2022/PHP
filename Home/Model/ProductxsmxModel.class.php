<?php
    ...
    use Think\Model;
    class ProductxsmxModel extends Model 
    {
        protected $tableName = 'wt_goodsxs_mx';
		...
		{
			$count=$this->alias('xsmx')->join('left join wt_goodsinfo info on info.goods_id=xsmx.shiji_goods_id and info.wbid=xsmx.wbid')->where($map)->count(); 

			$list= $this->alias('xsmx')->join('left join wt_goodsinfo info on info.goods_id=xsmx.shiji_goods_id and info.wbid=xsmx.wbid')
			->field(array(
			   'xsmx.wbid'=>'wbid',
			   'xsmx.shiji_goods_id'=>'goods_id',
			   'xsmx.post_order_no'=>'post_order_no',
			   'xsmx.xiaoshou_num'=>'sumnum',
			   'xsmx.je'=>'je',
