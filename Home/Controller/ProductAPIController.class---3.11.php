...
class ProductAPIController extends Controller
{
	...
	{	
		...
		else
        {
		  ...
		  if($shijiancha>0)
		  {
		   ...
		   if($yuangonginfo)
		   {		
	            $roleinfo=D('Role')->field('th_qx,sxj_qx,jch_qx,ch_qx,spinfo_qx,xiajia_qx')->where(array('WB_ID'=>$wbinfo['WBID'],'role_id'=>$yuangonginfo['role_id']))->find();  	   
		       /*   
				...
				*/
				...
				else
                {
					...
					$yuangonginfo['ch_qx']=$roleinfo['ch_qx'];
					$yuangonginfo['spinfo_qx']=$roleinfo['spinfo_qx'];
					$yuangonginfo['ck_qx']=$ck_qx;
					$yuangonginfo['xiajia_qx']=$roleinfo['xiajia_qx'];
					
					$wf_qx	 =D('WIni')->getOneRecordByName2($wbinfo['WBID'],'ShangPin_HyMaiSp');  
                    if(!empty($wf_qx)){
						$wf_qx=1;
					}else{
						$wf_qx=0;
					...
					$yuangonginfo['wf_qx']=$wf_qx;  //是否允许网费购买商品
	...
	public function API_query_goods_jiaobanlist()
	{
		...
		foreach($moneylist as &$val)
		{
            ...
            */
			...
			if($val['paytype']==4 && $val['ordertype']==1)
			{
				$list['sumje_wangfei_xs']=$val['sum_je'];
			}
					
		}
		
		if(empty($list['sumje_wangfei_xs']))
		{
			$list['sumje_wangfei_xs']=0;
		}else
        {
			$list['sumje_wangfei_xs']=(float)sprintf("%.2f", $list['sumje_wangfei_xs']); 
		}
		...
		*/
		...
		$list['sum_sr_je']=$list['sumje_cash_xs']+$list['sumje_cash_jbxs']-$list['sumje_cash_th']+$list['sumje_nocash_xs']-$list['sumje_nocash_th']+$list['sumje_other_xs']-$list['sumje_other_th']+$list['sumje_wangfei_xs'];
	...
	{	  
		...
		{				
			...
			foreach($moneylist as &$val)
			{
				...
				if($val['paytype']==4 && $val['ordertype']==1)
				{
					$list['sumje_wangfei_xs']=(float)sprintf("%.2f", $val['sum_je']);
				}
			}
	...
	public function API_client_buygoods()
	{
	   ...
	   // $xiaoshou_insert_data['bz']='付:';
		...
		$xiaoshou_insert_data['paytype']=$paytype;                       //现金支付
