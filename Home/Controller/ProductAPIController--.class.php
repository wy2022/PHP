...
class ProductAPIController extends Controller
{
	...
	{	
        ...
        {				        
			...
			if(!empty($type_id))
			{
		 	...
		 	$list= D('Newproduct')->field('id,goods_id,type_id,goods_name,kc_num,xiaoshou_num,goods_image,shou_price,goods_pinyin,goods_quanpin')->where($map)->Order('id desc')->page($page,$rows)->select();			
		}
	...
	public function API_query_goods_jiaobanlist()
	{
		...
		$lastshifttime=date('Y-m-d H:i:s',strtotime($lastshiftinfo['dtEndTime']));
		//获取商品列表
		$all_goods_list=D('Newproduct')->field('id,wbid,goods_id,type_id,shou_price,goods_name,kc_num,ku_num_temp')->where(array('wbid'=>$wbid,'deleted'=>0))->order(' shou_price asc ')->select();
		...
		foreach($all_goods_list as &$val)
		{
			...
			else
            {
				...
				$shiji_goods_list[$i]['kc_num_temp']=$val['kc_num_temp'];
	...
	public function API_client_buygoods()
	{
		...
		foreach($xiaoshou_goodslist as &$val)
		{
			...
			$list[$j]['sellNum']=$val['xiaoshou_num'];
			$list[$j]['shou_price']=$val['price'];
			$list[$j]['totalPrice']=$val['qianshu'];
			$list[$j]['type_id']   =$val['typeid'];
			$list[$j]['kc_num']=0;
			$sumnum+=$val['xiaoshou_num'];			
			$j++;
		}
	   ...
	   // $xiaoshou_insert_data['bz']='付:';
		...
		$xiaoshou_insert_data['sumnum']=$sumnum;
		$xiaoshou_insert_data['detailinfo']=json_encode($list);												
	...
	public  function   fnGetSaomaDianhuoNum()   //扫码时候读取所有商品库存到界面
	{
        ...
        {		
			...
			else
            {
		 	...
		 	$list= D('Newproduct')->field('id,goods_id,goods_name,kc_num,ku_num_temp')->Order('id desc')->where($map)->select();     		
		}					
		$list2['shiji_goods_list']=$list;
        $list2['result']=1;
        $list2['flag']=$flag;
